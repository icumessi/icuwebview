> **Note:** This README was generated by AI since I believe AI would do a MUCH better job at explaining the usage than me lmao. (I gave the AI a large amount of context before, so do NOT worry lmao)
...
greet_cb :: proc "c" (seq, req: cstring, arg: rawptr) {
    ...
    app := (^App)(arg)
    // req is a JSON array — use json_arg to pull values out by index
    name, _ := json_arg(string(req), 0)
    app.counter += 1
    // Strings returned to JS must be JSON-encoded (%q adds the quotes)
...
increment_cb :: proc "c" (seq, req: cstring, arg: rawptr) {
    ...
    app := (^App)(arg)
    app.counter += 1
    ui.return_val(seq, fmt.ctprintf("%d", app.counter))  // numbers need no quotes
}
...
main :: proc() {
    ...
    ui.set_html(HTML)
    ui.run()   // blocks until closed or terminate() is called
}
```
...
Conversion to `cstring` happens internally via the temp allocator (a bump-pointer alloc + memcpy — effectively free). Two intentional exceptions still use `cstring`:
...
| `eval_result(js: string, w = nil) -> string` | Execute JS and block until the result comes back (5 s timeout). Returns a JSON-encoded string. **Do not call from inside a bind callback** — it will deadlock. |
...
| Proc | Description |
|---|---|
| `bind(name: string, fn, arg = nil, w = nil)` | Bind `window._icu.<name>(...)` to an Odin callback. |
...
| Proc | Description |
|---|---|
| `set_data_dir(path: string)` | Set persistent browser data dir. Call before `create()`. `""` = ephemeral temp dir (default). |
...
| `load_from(path: string) -> bool` | Load DLL from a specific path. |
...
`req` in your callbacks is a JSON array string like `["Alice", 42, true]`. Use the bundled `json_arg` helper:
